...
function Dashboard({ user }: { user: SessionUser }) {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      // load active account settings (currency + starting balance)
      if (Number.isFinite(accId)) {
        ...
        if (acc?.starting_balance !== undefined && acc?.starting_balance !== null) {
          ...
          if (Number.isFinite(sb)) setStartingBalance(sb);
        }
      }
  ...
  const pctReturn = startingBalance > 0 ? (monthTotal / startingBalance) * 100 : null;
  ...
  const saveToday = async () => {
    ...
    // Update local month map + recompute total fast
    setMonthMap((prev) => {
    ...
    // (month total already updated above)
  };

  const saveStartingBalance = async () => {
    if (!activeAccountId) {
      setStatus("Create/select an account first (Pricing → Pro enables multi-accounts)." );
      return;
    }
    ...
    if (error) setStatus(`Starting balance save failed: ${error.message}`);
    else setStatus("Starting balance saved ✅");
  };
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="rounded-3xl border border-white/10 bg-white/5 p-5">
          ...
          <button
            ...
            className="mt-2 w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm hover:bg-white/10"
          >
            Generate Trader Card (share)
          </button>
          ...
          <div className="flex flex-wrap items-end justify-between gap-3">
            ...
            <div className="min-w-[220px]">
              ...
              <div className="mt-3 rounded-2xl border border-white/10 bg-white/5 p-3">
                <div className="flex items-center justify-between text-xs text-[rgb(var(--muted))]">
                  <span>Starting balance (for %)</span>
                  <span className="text-[rgba(255,255,255,0.85)]">{currency}</span>
                </div>
                <div className="mt-2 flex gap-2">
                  <input
                ...
                <div className="mt-2 text-[11px] text-[rgb(var(--muted))]">
                  {pctReturn === null ? "" : `This month: ${pctReturn >= 0 ? "+" : ""}${pctReturn.toFixed(2)}%`}
                </div>
              </div>
            </div>
          </div>
